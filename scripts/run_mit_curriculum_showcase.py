...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run engine on MIT curriculum datasets and summarize insights.")
    parser.add_argument("--datasets-dir", default="datasets/mit_curriculum_datasets", help="Directory with dataset zips.")
    parser.add_argument("--out-dir", default="reports/mit_curriculum_insights", help="Directory for generated insight reports.")
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=[],
        help="Optional signal heads to enable (e.g., monte_carlo forecast regime_change).",
    )
    ...
    zip_paths = sorted(datasets_dir.glob("*.zip"))
    if not zip_paths:
        raise SystemExit(f"No datasets found in {datasets_dir}")

    if args.policy == "identity":
        policy = IdentityPolicy()
    else:
        policy = CapacityPolicy(PolicyConfig(max_edges=args.max_edges, sticky_fraction=args.sticky_fraction))

    signal_config = SignalConfig(
        compute_spread=args.compute_spread,
        compute_locality=args.compute_locality,
    )
