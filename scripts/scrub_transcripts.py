...
def parse_transcript(lines: List[str]) -> Dict:
    ...
    for ln in lines:
        ...
        if m:
            ...
            # continuation of previous speaker or context
            if current_speaker is not None:
                buffer.append(ln)
            else:
                # treat as context line (narration)
...
def summarize_context(lines: List[str]) -> str:
    # crude heuristic: take the first non-empty non-artifact lines up to ~2 sentences
    text = " ".join(lines[:20])
